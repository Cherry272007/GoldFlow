...
function AnalyzeButton({ busy, disabledLabel, idleLabel, onClick, disabled }) {
  return (
    <button
      onClick={onClick}
...
export default function AIPage({ market, status, onBack }) {
  ...
  return (
    <div className="mx-auto min-h-screen max-w-3xl px-4 pb-44">
      ...
      <section className="mb-3 rounded-2xl border border-line bg-gradient-to-b from-card to-card2 p-4">
        ...
        <p className="mb-3 text-xs leading-relaxed text-muted">
          Run the AI verdict using the current live market data and indicators only.
        </p>
        <AnalyzeButton
          busy={goldBusy}
          disabled={false}
      ...
      <section className="mb-3 rounded-2xl border border-line bg-gradient-to-b from-card to-card2 p-4">
        <SectionHeading
          icon={
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
              <rect x="3" y="3" width="18" height="18" rx="3" stroke="#c9a45c" strokeWidth="1.5" />
              <circle cx="8.5" cy="8.5" r="1.5" fill="#c9a45c" />
              <path d="M3 16 L8 12 L13 16 L17 13 L21 16" stroke="#c9a45c" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          }
        >
          AI Image Analyze
